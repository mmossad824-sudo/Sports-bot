...
def post_fb_comment(video_id: str, message: str) -> bool:
    """Post a comment on a Facebook video."""
    if not FB_PAGE_TOKEN or not FB_PAGE_ID:
        return False
    try:
        url = f"https://graph.facebook.com/v21.0/{video_id}/comments"
        r = requests.post(url, data={"message": message, "access_token": FB_PAGE_TOKEN}, timeout=30)
        data = r.json()
        if r.status_code == 200 and data.get("id"):
            logger.info(f"✅ FB Comment posted on video {video_id}: id={data.get('id')}")
            return True
        else:
            logger.error(f"FB Comment failed: {r.status_code} — {r.text[:300]}")
            return False
    except Exception as e:
        logger.error(f"FB Comment exception: {e}")
        return False

def post_fb_video(description: str, video_path: str) -> str:
    """Post a short video or Reel to Facebook Page, and also to Stories. Returns video ID."""
    if not FB_PAGE_TOKEN or not FB_PAGE_ID:
        return None
